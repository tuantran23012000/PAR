...
def main(cfg, args):
    ...
    if cfg.DATASET.TYPE == 'pedes':
        ...
        # test_set = PedesAttr(cfg=cfg, split=cfg.DATASET.TEST_SPLIT, transform=valid_tsfm,
        #                       target_transform=cfg.DATASET.TARGETTRANSFORM)
    ...
    #train_sampler = 
    #print(train_sampler)
    # class_sample_count = train_set.label.sum(0)
    # weights = 1 / torch.Tensor(class_sample_count)
    # weights = weights.double()
    # #train_sampler = weights
    # train_sampler = torch.utils.data.sampler.WeightedRandomSampler(weights, cfg.TRAIN.BATCH_SIZE)
    ...
    if args.local_rank == 0:
        ...
        print(f'{cfg.DATASET.NAME} attr_num : {train_set.attr_num}, eval_attr_num : {train_set.eval_attr_num} '
              ...
              f'{cfg.DATASET.VAL_SPLIT} set: {len(valid_loader.dataset)}, '
              #f'{cfg.DATASET.TEST_SPLIT} set: {len(test_loader.dataset)}, '
              )
    ...
    if BALANCE_CLASS:
        ...
        criterion = build_loss(cfg.LOSS.TYPE)(
            sample_weight=None, scale=cfg.CLASSIFIER.SCALE, size_sum=cfg.LOSS.SIZESUM, tb_writer=writer,
            samples_per_cls=sample_class, no_of_classes=no_of_classes,beta=beta,gamma=gamma)
    else:
        criterion = build_loss(cfg.LOSS.TYPE)(
            sample_weight=label_ratio, scale=cfg.CLASSIFIER.SCALE, size_sum=cfg.LOSS.SIZESUM, tb_writer=writer,
            samples_per_cls=None, no_of_classes=None,beta=None,gamma=None)
...
def argument_parser():
    ...
    parser.add_argument(
        ...
        default="./configs/pedes_baseline/rap_zs_addition.yaml",
    )
